...
export default function Dashboard() {
  ...
  const registrosMes = datos.filter(r => r.fecha && r.fecha.startsWith(mesSeleccionado));

  // Agrupar ingresos (Ingresos y Ahorros)
  const ingresos = registrosMes.filter(r => r.categoria === 'Ingresos' || r.categoria === 'Ahorros');
  const ingresosPorSubcat = {};
  ingresos.forEach(r => {
    if (!ingresosPorSubcat[r.subcategoria]) ingresosPorSubcat[r.subcategoria] = 0;
    ingresosPorSubcat[r.subcategoria] += parseFloat(r.monto);
  });
  ...
  const egresos = registrosMes.filter(r => r.categoria === 'Gastos básicos' || r.categoria === 'Gastos deseo');
  const egresosPorSubcat = {};
  egresos.forEach(r => {
    if (!egresosPorSubcat[r.subcategoria]) egresosPorSubcat[r.subcategoria] = 0;
    egresosPorSubcat[r.subcategoria] += parseFloat(r.monto);
  });
